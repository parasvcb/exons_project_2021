...
def transcriptObcreator(genefk, transpk, trans, totc, uc, exons_fk_has, dom_trans_spans, col_cood_hash):
    ...
    for i in trans.exons:
        ...
        if i.seq:
            ...
            modExo += '%s,%s,%s_'%(factor_length,factor_length+len(i.seq),smoothDis)
...
def domseqiterator(trans, transfk, transdoms, genecodes, domainhash):
    ...
    if transdoms:
        for i in transdoms:
            ...
            # alphabet returns
            # seq span 500 length, dom 408 to 430
            # SHOULD BE  a should be 407 , dom length 23, 70
            # span of 408 to 430 is actually 407 to 429, 23 characters
            # a is stuff before that, means, 0:407, will be selected till 406, but 407 chrs
            # dom is 407 to 430, will be selected till 429, 
            # c id stuff after, 430 ahead
            
            domvar = (i[1]-i[0]) + 1
            # avar = i[0]-2 if i[0]-2 >=0 else 0
            avar = i[0]-2 if (i[0]-2) >=0 else 0
            cvar = i[1] -1 if (i[1]-1 )>=domvar else i[1]
            # go through cases of NP_001277944.1 and XP_011246388.2 of gene 71001 for detailed elaboration
            a = strdom[:avar]
            # sleects s
            c = strdom[cvar:]
            dom = (domvar)*domcode
            strdom = a + dom + c
            print ( "avar,Dom,Cvar", avar, domvar, cvar)
            print ("avra+domvar", len(a)+domvar)
            print (a)
            print ("a: ",len(a))
            print (dom)
            print ("dom: ",len(dom))
            print (c)
            print ("c: ",len(c))
            print (len(strdom), lengthInitial, len(strdom)!=lengthInitial)
            if len(strdom)!=lengthInitial:
                print ("Raise Flag, %s, has domseqiter length problme"%trans.ID)
                sys.exit()
            # crop and keep strdom till start of span , if span is 55,100, then keep :54 same, (100-55)+1 as domcood, and then 100 ahead to rest also as same 
            # -> this only poses some issues, what are they :O
