...
export class AllTranscripts extends Component {
  ...
  handleClick(tidinp) {
    ...
    if (ind !== -1) {
      // if element does exist in array, remove it
      arr.splice(ind, 1);
      this.setState({
        arraylis: arr
      });
    } else {
      //  trans doesnot exixt so add it, 
      this.setState({
  ...
  render() {
    ...
    let dropnighttext = this.state.dropDown ? "Hide" : "show info tracks"
    ...
    // }
    {console.log("(ArrayLis)", this.state.arraylis)}
    return (
      <>
        ...
        {this.state.arraylis.length > 0 ? (
          <div className="container-fluid align-items-center mt-5">
            ...
            {this.state.exonfull ? (
                ...
                <div className="row">
                  <div className="col-sm-9">
                    {this.state.arraylis.map(tr1 => (
                      <div className="m-0 p-2 overflow-auto" key={tr1.tId}>
                        <RenderExonsFull
              ...
              //       Dropdown button
              //     </button>
              <div className="row h-auto justify-content-center" aria-labelledby="dropdownMenuButton1">
                <div className="d-flex w-75 align-items-left square centered flex-column sticky-top p-2" style = {{background:"#f7f7f7"}}>
                  { this.state.dropDown ? 
                  <div className="row">
                    <div className = "col-1">
                      <div onClick={this.handleDropdown} button type="button" class="btn-close" disabled aria-label="Close">
                      </div>
                    </div>
                    <div className = "col-11">
                    <div className="p-1 m-1" style ={{fontSize: "12px", lineHeight: 0.8, color:"#a9a9a9" }}>
                        <p>follwing are the tracks and their sequence</p>
                        <p>1. sequqnce tracker and buttons which can be dragged with mouse and clciked and expanded</p>
                        <p> track 1: Alternate colors depict the exons in linear sequqnce for current transcript, zigzag end suggests not completely dcoding and has UTR component also</p>
                        <p> track 2: Shows secondary strcutre region in alphahelical blue rehions and beta sheet strands  </p>
                        <p> track 3: Shows disordered region</p>
                        <p> track 4: Shows domains</p>
                    </div> 
                  </div>                    
                  </div>
                    ...
                    <div className="row">
                      <div className = "col-12">
                        <span style ={{fontSize: "15px", lineHeight: 0.8, color:"black"}} onClick={this.handleDropdown} button type="button" class="btn-sm">
                          <i className="bi-plus-lg pr-2 mr-2">  &nbsp;&nbsp;
                          <span style = {{color:"#a9a9a9"}}> Show tracks information
                            </span>
                            </i>
                        </span>
                      </div>
