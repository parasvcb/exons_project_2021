...
export class RenderExonsAln extends Component {
  ...
  render() {
    ...
    return (
      ...
      >
        {exonset.map(tran => (
          <div key={tran.tid} style={{ display: "flex", padding: "0.5rem", align:"left" }}>
            <span>
              <span className="badge bg-success align-items-left" style={{ width: "150px" }}>
    ...
    if (typeid === "string") {
      if (exid[0] === "R") {
        ...
        switch (elemneeded) {
          case "n0":
            ob = (
              <span className="badge bg-light float-left border">{"<"}</span>
            );
            break;
          case "0c":
            ob = (
              <span className="badge bg-light float-right border">
                {">"}
              </span>
            );
            break;
