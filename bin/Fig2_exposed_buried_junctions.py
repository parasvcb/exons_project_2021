...
import sys,os
if len(sys.argv)!=6:
        print ("Please give 1 object 2 conditionFile 3 pdbresultsDir 4 naccessdir 5 results dir")
        sys.exit()
prog,source_gene_object,gene_conFile,  pdb_dir, naccess_dir, results_dir_csv = sys.argv
...
def sstype_ends_indi(gene_source,condition, filename, window,fout):
    ...
    for gene in desiredlistofgenes:
        ...
        if PI:
            ...
            #ss_seq calculation
            structurePI=PI.structure_fileAF
            naccessLink=os.path.join(naccess_dir,structurePI+'.rsa')
            proteinPDB_seq=am.givemeresnoofpdb(os.path.join(pdb_dir,structurePI))
            srfaceData,exonspanhas,ss_seq=am.rsaToSeqFile(proteinPDB_seq,PI,naccessLink)
            ...
            for i in range(0,len(PI.exons)-1):
                if PI.exons[i].length>9 and PI.exons[i+1].length>9:
                    #-> adding on more condition for the buried exposed residues
                    if set(exonspanhas[PI.exons[i]][-window:] + exonspanhas[PI.exons[i+1]][:window]) < set(proteinPDB_seq):
            ...
            if PI_representative:
                for i in PI_representative:
                    i1seq=am.subsetsurfaceexposedExonWise(srfaceData,exonspanhas[i[0]])
                    i2seq=am.subsetsurfaceexposedExonWise(srfaceData,exonspanhas[i[1]])
...
def sstype_ends_coservation(gene_source,condition, filename, window, fout):
    ...
    for gene in desiredlistofgenes:
            ...
            for trans in gene_source[gene].transcripts:
                representative= am.junctions_from_different_isoforms_poulatorSURFACEEXPOSED(representative, trans, trans.PI,window, pdb_dir, naccess_dir)
    ...
    am.sorting_screening_junctions(has_exon_categories_combined,filename+'WEF_and_exonsJunction.csv',filename+'WEF_and_majorSS_exonsJunction_conservation.csv', default_sstypes=['EE','BB'])
        
sstype_ends_coservation(humanGeneObject, CONDITION_GENES, results_dir_csv +'ExonsJunction_BuriedExposedWin3', 3,fileswriter)
sstype_ends_indi(humanGeneObject, CONDITION_GENES, results_dir_csv +'Individual_BuriedExposedWin3', 3,fileswriter)

sstype_ends_coservation(humanGeneObject, CONDITION_GENES, results_dir_csv +'ExonsJunction_BuriedExposedWin1', 1,fileswriter)
sstype_ends_indi(humanGeneObject, CONDITION_GENES, results_dir_csv +'Individual_BuriedExposedWin1', 1,fileswriter)
