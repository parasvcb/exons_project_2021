...
fileswriter=open(results_dir_csv+"csv/ss/ss.log","w")
window=4
...
sstype_ends_indi(has_gene, condition, results_dir_csv +'csv/ss/_Individual_', window,fileswriter)
...
def ss_def_customised( ex1, ex2, window):
    '''
    Get ex1, ex2 as secondary structres of the exons, return their common N and C ter residues as HCE or X
    '''
    n = ex2[:window-1]
    c = ex1[-(window-1):]
    if len(set(n)) == 1:
...
def conservation_junc_populator(repr_has, dup, trans,PI, window):
    # repr_has and dup are gene exon pairs (not in trancript butr total length perpsective pairs (0,len(exons)))
    '''
    for a trancript:
        ...
        if their length is grt 9 (both):
            ...
            if they are not retention cases and they are not empty ss's
                ...
                now get their junction sequqnces, both amino acid and SS
                if that junction 4tup pair is not there in dup(linear key), add it
...
def sstype_ends_coservation(gene_source,condition, filename, window, fout):
    ...
    screen transcripts, and for every trancript, get its aa sequence, and fro unique protein coding sequqnces only, populate the repr has and its duplicate like below:
        iterate its exons in pairs, 
        if their length is grt 9 (both):
            get their ss seqs
            if they are not retention cases and they are not empty ss's
                get their linear sequqnce tags or close to them ()
                now get their junction sequqnces, both amino acid and SS (4tup)
                if that junction 4tup pair is not there in dup(linear key), add it
                and make similar addition to repr_has version but add there in value (exon id of pairs, amino acid seq and PI tag if any)
            ...
            for i in representative:
                ...
                if len(representative[i])>2:
                    ...
                    for j in representative[i]:
                        #j is list, 0th elem is ex1Id, 1st is ex2ID, 2nd is SSseq, 3rd is SSseq2 then 4th is PI Flag)
                        seq1=j[2]
                        seq2=j[3]
...
sstype_ends_coservation(has_gene, condition, results_dir_csv +'csv/ss/_varying_partner_', window,fileswriter)
