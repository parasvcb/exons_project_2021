...
if len(sys.argv)!=3:
	print ("Please type correct location of object, results dir having condition pickle and gene of interest as well")
	sys.exit()
...
with open(os.path.join(results_dir_csv,"NCB_log.log"),"w")as fin:
    ...
    fin.write("Both_coding\t%s,%s\t%s,%s\t%s,%s\n"%(cod_bothN['n'],round(float(cod_bothN['n'])/sumtotn,3),
                                                       cod_bothN['c'],round(float(cod_bothN['c'])/sumtotc,3),
                                                       cod_bothN['b'],round(float(cod_bothN['b'])/sumtotb,3)))
    fin.write("CParent_NCchild\t%s,%s\t%s,%s\t%s,%s\n"%(cod_parent_noncood_childN['n'],round(float(cod_parent_noncood_childN['n'])/sumtotn,3),
                                                       cod_parent_noncood_childN['c'],round(float(cod_parent_noncood_childN['c'])/sumtotc,3),
                                                       cod_parent_noncood_childN['b'],round(float(cod_parent_noncood_childN['b'])/sumtotb,3)))
    fin.write("NCParent_Cchild\t%s,%s\t%s,%s\t%s,%s\n"%(cod_child_noncood_parentN['n'],round(float(cod_child_noncood_parentN['n'])/sumtotn,3),
                                                       cod_child_noncood_parentN['c'],round(float(cod_child_noncood_parentN['c'])/sumtotc,3),
                                                       cod_child_noncood_parentN['b'],round(float(cod_child_noncood_parentN['b'])/sumtotb,3)))
    fin.write("Both_NonCoding\t%s,%s\t%s,%s\t%s,%s\n"%(non_cood_bothN['n'],round(float(non_cood_bothN['n'])/sumtotn,3),
                                                       non_cood_bothN['c'],round(float(non_cood_bothN['c'])/sumtotc,3),
                                                       non_cood_bothN['b'],round(float(non_cood_bothN['b'])/sumtotb,3)))
    ...
    fin.write("\n\nN segregation\nType\tTotal\tFreq\n")
    for keys in keyvals:
        fin.write("%s\t%s\t%s\n"%(keys,n_coding_both_segregation[keys],                                  round(float(n_coding_both_segregation[keys])/sumN,2)))
    fin.write("\n\nC segregation\nType\tTotal\tFreq\n")
    for keys in keyvals:
        fin.write("%s\t%s\t%s\n"%(keys,c_coding_both_segregation[keys],                                  round(float(c_coding_both_segregation[keys])/sumC,2)))
    fin.write("\n\nB segregation\nType\tTotal\tFreq\n")
    for keys in keyvals:
        fin.write("%s\t%s\t%s\n"%(keys,b_coding_both_segregation[keys],                                  round(float(b_coding_both_segregation[keys])/sumB,2)))
    fin.write("\n\nFor N from total relevant :%s cases %s(%s) have length change of -2 to 2"%    (n_coding_both_segregation['relevant'],N_zero,round(float(N_zero)/ n_coding_both_segregation['relevant'],3)))
    fin.write("\nFor C from total relevant :%s cases %s(%s) have length change of -2 to 2"%    (c_coding_both_segregation['relevant'],C_zero,round(float(C_zero)/ c_coding_both_segregation['relevant'],3)))
    fin.write("\nFor B from total relevant :%s cases %s(%s) have length change of -2 to 2"%    (b_coding_both_segregation['relevant'],B_zero,round(float(B_zero)/ b_coding_both_segregation['relevant'],3)))
...
with open (os.path.join(results_dir_csv,"matrix_NCBI.csv"),"w") as fin:
