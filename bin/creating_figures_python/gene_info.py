...
def formatter(gname,domname,exonsLis,codingexonslis,domcontribNormal, excontribNormal,domcontribcons, excontribcons):
    ...
    print '\nlengthmatch',normL==consL
    print '\nDomainPerspective:'
    exDom=domcontribNormal
    for ex in exDom:
        print ex[0]
        for keys in ex[1].keys():
            print keys,ex[1][keys]
        print '*\n'
    print '\n'
    print 'DomainPerspective_conseCoding:'
    exDom=domcontribcons
    exDom.sort()
    for ex in exDom:
        print ex[0]
        for keys in ex[1].keys():
...
def domain_exon_fraction(human):
    ...
    for gene in human:
        ...
        if domlis and exlis:
            ...
            if 1:
                test(human[gene])
                formatter(gene,domain_namewith_cood,exlis,consecCoding, gene_exons_domains_fractions[gene]['domains'],gene_exons_domains_fractions[gene]['exons'],gene_exons_domains_fractions[gene]['domains_conseCoding'], gene_exons_domains_fractions[gene]['exons_conseCoding'])
...
def test(geneob):
    relev_exons=[]
    print geneob.ID,geneob.detail,geneob.localization
    #print ([t.ID for t in geneob.transcripts])
    for t in geneob.transcripts:
        print t.ID,t.PI, ", ".join([ex.ID for ex in t.exons])
    print "Exons principal isoforms:"
    for ex in geneob.PI.exons:
        print ex.ID, ex.length
        key=ex.ID.split('.')[2] if ex.ID[0]!='R' else 'A'
        key='A' if key == 'F' else key
        #print (key)
        if ex.length>0:
            relev_exons+=[[ex.length,key,ex.ID]]# -2val: include in analysis as 1 and remove in analysis as 0, -1val:0 
    print 'relev_exons:',relev_exons

    domainsTranscript=geneob.PI.cath_list if CONST_val=='cath' else geneob.PI.pfam_list
    print 'DomainsInPI:',domainsTranscript
    print '\n**'
