...
def compileExonData (fname, string, organism, mode = "basic"):
    with open (fname) as fin:
        dat=[i for i in fin.read().split('\n') if len(i)>1]
    '''
    ...
    for i in dat[1:]:
        ...
        # if categ in ["Total_exons","Total_exons_ncb&R","UTRStrict","UTRAlternate","UTRAlternateWithSS","UTRMajorlyConstitutive","UTRConstitutive","CodingStrict","CodingAlternate","CodingAltWaachange","CodingAlternateWithSS","CodingMajorlyConstitutive","CodingConstitutive","CodingConstitutiveWaachange","DualStrict","DualAlternate","DualAltWaachange","DualAlternateWithSS","DualMajorlyConstitutive","DualConstitutive","DualConstitutiveWaachange","Intron_retained_coding","Intron_retained_UTR","StrictMcases(1ntLength)","Strict_aa_removed"]:
        facet = "4_coding" if "coding" in categ else "2_UTR" if "UTR" in categ else "1_Dual" if "Dual" in categ else "0_extras"
        if mode == "basic":
            string += "%s\t%s\t%s\t%s\t%s\n"%(categ, facet, organism, Mean, Stddev)
        else:
            ...
            string += "%s\t%s\t%s\t%s\t%s\n" % (categ, facet, organism, exFrac, geFrac)
    return string
...
def compileTrans (fname, string, organism):
    ...
    for i in dat[1:]:
        ...
        if "Background" not in categ:
            categ = "Total_ISF" if "All_transcripts_statistics (including redundant" in categ else "NR_Isf" if "All_transcripts_statistics (only nonredundantPer gene" in categ else "UTR_per_NR_Isf" if "Per_proteinISF_UTR_var" in categ else "MaxUTRForIsf/gene"
...
hasTxid = {'10090': 'Mouse', '9606':'Human', '7227':'Fly', '7955': 'Fish', '6239':'Worm' }

stringA1_GT_exonData = 'Category\tOrganism\tValue\n'
stringA2_GT_transData = 'Category\tOrganism\tValue\n'


stringB1_exonData_meanStdCounts = 'Category\tFacet\tOrganism\tValueMean\tValueStd\n'
stringB1_exonData_exonGeneFrac = 'Category\tFacet\tOrganism\tValueFractionExons\tValueFractionGenes\n'

stringC_exonLength = 'Category\tOrganism\tValueMean\tValueMedian\tValueStd\n'

stringD_exonWEF = 'Category\tOrganism\tFrequency\tTotal\n'
...
for i in dat:
    if len(i) > 0:
        ...
        stringB1_exonData_meanStdCounts = compileExonData (os.path.join(i,fprefix, "tabular_ExonsData.csv"), stringB1_exonData_meanStdCounts, org)
        stringB1_exonData_exonGeneFrac = compileExonData (os.path.join(i,fprefix, "tabular_ExonsData.csv"), stringB1_exonData_exonGeneFrac, org, mode = "fraction")
