...
def sub_aachangecases(key, exon_pack, cod_status_pre, coding_cood, tupRangeExoni, exons_aa_change, parent_or_alt_exons, parent_exons_eve):
    ...
    parentImitation = (exon_pack[0], temp_exonListParentOrAlt[exon_pack[0]][0], temp_exonListParentOrAlt[exon_pack[0]][1])
...
def hidden_events1(exon_pack, coding_cood, tupRangeExoni, parent_exons, alt_exons, parent_exons_eve, exons_aa_change):
    ...
    overlap = [i for i in parExonsSign if set(tupRangeExoni) & set(parent_exons[i][tupleSpanIndex])]
    ...
    if len(overlap) == 1:
        ...
        if cside == 0 and abs(nside):
            ...
            newid = [parent_exon_cod_type, cod_status_pre, 'A', parent_exon_rank, 'n', event_occurence]
        ...
        for bspan in parent_exons_eve[first_parent_exon][2].keys():
            ...
            if leftChange not in hasLeftBformer:
                hasLeftBformer[leftChange] = []  # nside change as key, and complete ID as value, ID is []
        ...
        for bspan in parent_exons_eve[last_parent_exon][2].keys():
            ...
            if rightChange not in hasRightBlatter:
                hasRightBlatter[rightChange] = []  # cside change as key, and complete ID as value, ID is []
            hasRightBlatter[rightChange] += [parent_exons_eve[last_parent_exon][2][bspan]]
...
def insertion_filter(insertion_cases, extension_cases, parent_exons_template):
    ...
    return empty insertion cases, added exnetsion cases and parente xons emplate
    '''
...
def exon_manipulator(raw_var, parent_exons, parent_exons_eve, isoforms):
    ...
    for exon in parent_exons:
        # exon is 3 ele tuple, and valtemp is values
        valtemp = parent_exons[exon]
        if valtemp[0][2] == 'A' and exon[0] not in template_temp_repeat_count:
            template_temp += [valtemp[0][3]] * raw_var.count(exon[0])
    ...
    # category F added, now it should work i think
