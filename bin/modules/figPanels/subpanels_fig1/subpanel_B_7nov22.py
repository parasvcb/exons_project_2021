...
def transcripts_type(has,fout,results_dir_csv, lengthThreshold=3000, UniqueTransCount=2, UniqueCodingExonCount=2 ):
    ...
    array_to_write+=["-> Genes with more than 1 listed isoform but only %s unique protein coding sequences are : %s"%(len(background_UTR), UniqueTransCount-1)]
    ...
    array_to_write+=["-> Genes with PI len less than 3000 aa and atleast %s different protein isoforms are :%s, amongst those, with atleast %s coding exons are: %s" %(UniqueTransCount, UniqueCodingExonCount, len(total_transcripts)+len(genes_gt1_isf_and_1_exon), len(CONDITION_GENES))]
    array_to_write+=[linesep]
    ...
    array_to_write+=["\tAll_transcripts_statistics (including redundant, but passed criteria)\t%s"%cm.stats(total_transcripts)]
