'''
version: previous version of 1 nov was doing calculation fo the splice sites changes and pure with weird logic and held back now, 
This verison has improved those calculation but still relies on exons mentioned in the gene.exons category instead if goig by the placeholder anchors
Next version will have the improvemenets
'''
'''
...
def exon_counting(has,res_dir,condition,fout ):
    ...
    for gene in has:
        if  gene in condition:
            ...
            conditionT = UTRStrict[-1]!=(len(tempExMajorUTR)+len(tempExAltUTR)+UTRConstitutive[-1])
            # print (UTRStrict[-1], len(tempExMajorUTR), len(tempExAltUTR), UTRConstitutive[-1], conditionT)
            if conditionT:
                print ("totUTR count", counter([i for i in has[gene].exons if re.match(r'^U',i.ID)],PARAMETER))
                print ("totUTR var", [i for i in has[gene].exons if re.match(r'^U',i.ID)])
                print ("tempExAltUTR", len(tempExAltUTR), tempExAltUTR)
                print ("tempExmajorUTR", len(tempExMajorUTR), tempExMajorUTR)
                print ("UTRConst COUnt",counter([i for i in has[gene].exons if re.match(r'^[U]\.[-][2]+\.G',i.ID)],PARAMETER))
                print ("UTRConst Var",[i for i in has[gene].exons if re.match(r'^[U]\.[-][2]+\.G',i.ID)])
            ...
            # tempUTRCons = [i for i in has[gene].exons if re.match(r'^[U]\.[-][2]+\.G',i.ID)]
            # tempUtrTot= [i for i in has[gene].exons if re.match(r'^U',i.ID)]
            # tempCond=len(tempExMajorUTR)+len(tempExAltUTR) + counter(tempUTRCons,PARAMETER)
            # if tempCond!=len(tempUtrTot):
            #     print (tempCond,UTRStrict, tempCond==UTRStrict)
            #     print ("UTRStrict", [i.ID for i in has[gene].exons if re.match(r'^U',i.ID)])
            #     print ("tempExMajorUTR", [i.ID for i in tempExMajorUTR])
            #     print ("tempExAltUTR", [i.ID for i in tempExAltUTR])
            #     print ("UTRConstitutive", [i.ID for i in has[gene].exons if re.match(r'^[U]\.[-][2]+\.G',i.ID)])
            #     sys.exit()

            if tempExMajorUTR:
                # res1,res2=ca.split_cases(tempExMajorUTR)
                # if res1:
                #     print ([i.ID for i in tempExMajorUTR])
                #     print (res1, res2)
                # UTRMajorlyConstitutive+=[res2]
                UTRMajorlyConstitutive+=[len(tempExMajorUTR)]
            
            if len(tempExAltUTR):
                # print ([i.ID for i in tempExAltUTR])
                res1,res2=ca.split_cases(tempExAltUTR)
                
                # print (res1,res2)
                # sys.exit()
                if res1+res2 != len(tempExAltUTR):
                    print ([i.ID for i in tempExAltUTR])
                    print (res1,res2)
            ...
            if tempExMajorDual:
                ...
                DualMajorlyConstitutive+=[res2] #counting forms n1, n2 as separate counts
            ...
            #its output will give in first, count of exons with no splice site change and 0 status, and sexond,  exons with chnage in splice sites, counted once for all variation ( T.1.A.4.n.1, T.1.A.4.n.2, will be counted as 1, same for their C and B's )
