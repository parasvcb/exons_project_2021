...
if (length(args)<3) {
  stop("Please supply two arguements 1. Dir and 2. inpPanelsString 3. afterExt(atit,core,both)", call.=FALSE)
...
df1 = read.csv(paste0(file.path(systemFile, "F2_Individual"),string_panels,"alljunct_window3.csv", tagAtit))
df1$Type <- "1_alljunct"
df2 = read.csv(paste0(file.path(systemFile, "F2_Individual"),string_panels,"background_window3.csv", tagAtit))
df2$Type <- "2_background"
df3 = read.csv(paste0(file.path(systemFile, "F2_Individual"),string_panels,"AltAlt_window3.csv", tagAtit))
df3$Type <- "3_AltAlt"
df4 = read.csv(paste0(file.path(systemFile, "F2_Individual"),string_panels,"ConsAlt_window3.csv", tagAtit))
df4$Type <- "4_ConsAlt"
df5 = read.csv(paste0(file.path(systemFile, "F2_Individual"),string_panels,"ConsCons_window3.csv", tagAtit))
df5$Type <- "5_ConsCons"
df6 = read.csv(paste0(file.path(systemFile, "F2_Individual"),string_panels,"ConsCons_window3.csv", tagAtit))
df6$Type <- "6_ConsCons"
...
ggsave(p1,filename=file.path(systemFile,paste0("junctions_PI_",tagAtit,".pdf")))